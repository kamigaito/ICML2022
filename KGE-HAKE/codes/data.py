...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        return pos_triple, neg_triples, self.hr_freq[(head, rel)], self.tr_freq[(tail, rel)], self.batch_type

    @staticmethod
    def collate_fn(data):
        positive_sample = torch.stack([_[0] for _ in data], dim=0)
        negative_sample = torch.stack([_[1] for _ in data], dim=0)
        #hr_freq = torch.cat(torch.Tensor([_[2] for _ in data]), dim=0)
        hr_freq = torch.Tensor([_[2] for _ in data])
        #tr_freq = torch.cat(torch.Tensor([_[3] for _ in data]), dim=0)
